...
<%@ page import="it.unibo.sdls.sampleproject.dao.hibernate.*" %>
...
TxDAOFactory daoFactory = (TxDAOFactory) request.getAttribute("daoTxFactory"); 
PublisherDAO publisherDAO = daoFactory.getPublisherDAO();
AuthorDAO authorDAO = daoFactory.getAuthorDAO();
BookDAO bookDAO = daoFactory.getBookDAO();

String operation = request.getParameter("operation");
boolean insertedPublisher = false;
	...
	<body>
		...
		<div>
			...
			<form>
				...
				<%= insertedPublisher ? "(publisher inserted correctly)" : "" %>
			</form>
		</div>
		...
		<div>
			...
			<form>
				...
				<input type="submit" name="submit" value="submit"/>
				<%= insertedAuthor ? "(author inserted correctly)" : "" %>
			</form>
		</div>
<%
...
%>
				...
				<input type="submit" name="submit" value="submit"/>
				<%= addedBook ? "(book added correctly)" : "" %>
			</form>
		</div>
<%
}// end if
...
%>
		<div>
			...
			<table>
				...
				<%= printTableRows( bookDAO.getAllBooks(), request.getContextPath() ) %>
			</table>
		</div>
